...
def main_code():
    ...
    global seedset_value
    if seedset == 1:
        vq_parser.set_defaults(seed = seedset_value)
    
    # epoch setting
    vq_parser.set_defaults(prompts = text)
       
    

    if in_image != "":
        vq_parser.set_defaults(init_image = str(in_image))
        vq_parser.set_defaults(max_iterations = 40)
        vq_parser.set_defaults(seed = 10)
        vq_parser.set_defaults(step_size = 0.02)
...
#UI control
class myMainWindow(QMainWindow, MainUI.Ui_MainWindow):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.PushButton.clicked.connect(self.onButtonClick_run)
        self.PushButton_2.clicked.connect(self.onButtonClick_deploy)
        self.PushButton_3.clicked.connect(self.select_image)
        self.checkBox.clicked.connect(self.onCheckBox1Click_seedset)
        self.thread = Worker()
        
        self.thread._signal.connect(self.call_backlog)  # 回傳GUI的事件
    ...
    def onButtonClick_deploy(self):
        ipfs_upload.photo_ipfs()
        subprocess.run(["brownie", "run", "./scripts/deploy_mint.py"])

    def run_print(self):
        
        self.label.setText("seed:"+seedset_value)
        img = QtGui.QImage("output.png")                 # 讀取圖片
        self.label_2.setPixmap(QtGui.QPixmap.fromImage(img))   # 加入圖片

    def onCheckBox1Click_seedset(self):
        global seedset
    ...
    def select_image(self):
        global in_image
        in_image, filterType = QtWidgets.QFileDialog.getOpenFileName()
        


    def call_backlog(self):
        global seedset_value
        self.label.setText("seed:"+str(seedset_value))
        img = QtGui.QImage("output.png")                 # 讀取圖片
        self.label_2.setPixmap(QtGui.QPixmap.fromImage(img))   # 加入圖片
